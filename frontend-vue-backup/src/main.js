...
// Prevent tree-shaking of chat button module
window.__createChatButton = createChatButton
...
let mqttClient = null
// Make latestData global for chat access
window.latestData = {
  'zenzero/wwt01': {},
  'zenzero/wwt02': {}
}
...
// Initialize dashboard and chat on page load (wait for DOM to be ready)
...
setTimeout(() => {
  ...
  // Initialize chat button after DOM is fully ready
  console.log('[Init] Initializing chat button...')
  try {
    const chatBot = createChatButton()
    if (chatBot) {
      console.log('[Init] Chat button initialized successfully:', chatBot)
    } else {
      console.error('[Init] Chat button initialization returned null/undefined')
    }
  } catch (error) {
    console.error('[Init] Error initializing chat button:', error)
  }
}, 300)
